...
static bool parse_int_from_str(int* value, char const* str) {
    ...
    char* end                = NULL;
    errno                    = 0;
    long const val           = strtol(str, &end, 0); // auto-detect base
    bool const range_error   = (val == LONG_MIN || val == LONG_MAX) && errno == ERANGE;
    bool const no_conversion = val == 0 && str == end;
    *value                   = (int) val;
...
static bool parse_double_from_str(double* value, char const* str) {
    ...
    char* end                = NULL;
    errno                    = 0;
    double const val         = strtod(str, &end);
    bool const range_error   = val == HUGE_VAL && errno == ERANGE;
    bool const no_conversion = val == 0.0 && str == end;
    *value                   = val;
    return !range_error && !no_conversion;
}
...
static void cli_param_value_print(cli_param_value_type_t type, cli_param_value_t value) {
    switch (type) {
        case STRING: printf("%s", value.STRING_value ? value.STRING_value : ""); break;
        case INT:    printf("%d", value.INT_value); break;
        case DOUBLE: printf("%f", value.DOUBLE_value); break;
        case BOOL:   printf("%s", value.BOOL_value ? "true" : "false"); break;
    }
}
...
static bool cli_param_value_init(cli_param_value_type_t type,
    ...
    switch (type) {
        case STRING:
            ...
            success           = true;
            break;
        case INT:    success = parse_int_from_str(&out->INT_value, argument); break;
        case DOUBLE: success = parse_double_from_str(&out->DOUBLE_value, argument); break;
        case BOOL:   success = parse_bool_from_str(&out->BOOL_value, argument); break;
    }
    return success;
}
...
static size_t cli_param_value_type_size(cli_param_value_type_t type) {
    switch (type) {
        case STRING: return sizeof(char const*);
...
static void* cli_param_transformed_values(cli_param_t const* param, cli_param_value_type_t type) {
    ...
    if (!param->uses_multiple_values) {
        switch (type) {
            case STRING: *(char const**) transformed = param->contained.STRING_value; break;
            case INT:    *(int*) transformed = param->contained.INT_value; break;
            case DOUBLE: *(double*) transformed = param->contained.DOUBLE_value; break;
            case BOOL:   *(bool*) transformed = param->contained.BOOL_value; break;
        }
    } else {
        for (size_t i = 0; i < param->values_len; ++i) {
            switch (type) {
                case STRING:
                    ...
                    break;
                case INT: ((int*) transformed)[i] = param->contained.values[i].INT_value; break;
                case DOUBLE:
                    ((double*) transformed)[i] = param->contained.values[i].DOUBLE_value;
                    break;
                case BOOL: ((bool*) transformed)[i] = param->contained.values[i].BOOL_value; break;
            }
        }
    }
    return transformed;
